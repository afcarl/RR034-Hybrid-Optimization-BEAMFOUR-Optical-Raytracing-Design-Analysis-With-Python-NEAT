...
class AdjHost implements B4constants
{
    ...
    public AdjHost() // constructor; performs the entire task. 
    {
        ...
        if (ngood < 1) 
        {
            ...
            try{
                PrintWriter writer = new PrintWriter("BEAM4_DONE.out", "UTF-8");
                writer.println( "Auto: no good rays" );
                writer.println( "Iteration = 9999" );
                writer.println( "RMS Average =  9999.0" );
                writer.println( "Nrays = 0" );
                writer.println( "Ngoals = 0" );
                writer.println( "Nterms = 0" );
                writer.println( "Nadj = 0" );
                writer.close();
            } catch (IOException e) {
               // do something
            }
        ...
        public void actionPerformed(ActionEvent ae)
        {
            ...
            if (bComplete)
            {
                ...
                try{
                    PrintWriter writer = new PrintWriter("BEAM4_DONE.out", "UTF-8");
                    writer.println( "Done" );
                    writer.println( "Iteration = " + hostiter );
                    writer.println( "RMS Average =  "+U.fwe(rms) );
                    writer.println( "Nrays = "+U.fwi(ngood,8).trim() );
                    writer.println( "Ngoals = "+U.fwi(autongoals, 1) );
                    writer.println( "Nterms = "+U.fwi(npts,8).trim() );
                    writer.println( "Nadj = "+U.fwi(nadj,8).trim() );
                    writer.close();
                } catch (IOException e) {
                   // do something
                }
